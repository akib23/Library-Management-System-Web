...
function closeNav() {
  ...
  document.body.style.backgroundColor = "white";
	...
	<div >
		...
		<form action="" method="post">
			<div>
				...
				<br>
				<br>
				<button style="background-color: white;color: black;float: right; margin: auto; margin-right:70px" type="submit" name="return" class="btn btn-default">Return Book
				</button>
				<input style="width: 300px;float: right; margin: auto; margin-right:10px;" class="form-control" type="text" name="bid" placeholder="Book ID">
	...
	<div>
		...
		<br>
		<br>
		<h1 style="color:white; margin-left:30px; ">Issued Books</h1>
		<br>
	...
	die("connection failed".mysqli_connect_error());
			...
			if(isset($_POST['submit']))
		{
			...
			$q=mysqli_query($db,"SELECT * FROM `issue_book` where username='$_SESSION[login_user]' and bname like '%$_POST[search]%' and NOT (issueb <=> 'NO'); ");
	...
	else
		{
			...
			$q=mysqli_query($db,"SELECT * FROM `issue_book` where username='$_SESSION[login_user]' and NOT (issueb <=> 'NO'); ");

			if(mysqli_num_rows($q)==0)
			{
				echo "<h3 style=color:red;>There is No Issued Books</h3>";
			}
			else
			{
				echo "<table class='table table-bordered'>";
	...
	if(isset($_POST['return']))
		{
			...
			mysqli_query($db,"DELETE from issue_book where username='$_SESSION[login_user]' and bid = '$_POST[bid]' and NOT (issueb <=> 'NO'); ");
